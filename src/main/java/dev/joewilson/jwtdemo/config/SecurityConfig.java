package dev.joewilson.jwtdemo.config;
...
public class SecurityConfig {
  ...
  public InMemoryUserDetailsManager user() {
    return new InMemoryUserDetailsManager(
            User.withUsername("joewilson")
                    .password("{noop}password") // user 'password' in popup login localhost:8080
                    .authorities("read")
                    .build()
    );
  }
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    return http
              ...
              //     .anyRequest().authenticated() // deprecated
              //   )
